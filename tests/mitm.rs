...
#[tokio::test]
async fn loaded_root_from_pem_client_verification() {
    let authority = MitmAuthority::generate("modeltap test root").unwrap();
    let cert_pem = authority.root_certificate_pem().unwrap();
    let key_pem = authority.root_private_key_pem().unwrap();
    let loaded = MitmAuthority::from_pem(&cert_pem, &key_pem).unwrap();

    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    let server_config = loaded.server_config_for("api.openai.com").unwrap();
    let server = tokio::spawn(async move {
        let (stream, _) = listener.accept().await.unwrap();
        let mut stream = TlsAcceptor::from(Arc::new(server_config))
            .accept(stream)
    ...
    let mut roots = RootCertStore::empty();
    roots.add(loaded.root_certificate()).unwrap();
    let client_config = ClientConfig::builder()
        .with_root_certificates(roots)
        .with_no_client_auth();
    let mut client = TlsConnector::from(Arc::new(client_config))
